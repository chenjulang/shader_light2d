...
export default class Rain extends cc.Component {
    ...
    //(x,y,0)*(5,5,0)=(0-0,0,x*5-y*5)

    _material: any;
    start() {
        this._material = this.node.getComponent(cc.Sprite).sharedMaterials[0];

        let aspect = new cc.Vec3(5.,.0,5.);
        let xy = new cc.Vec2(0,0);

        this.node.on(cc.Node.EventType.MOUSE_MOVE,(event: cc.Event.EventTouch) => {
            let pos = event.getLocation();
            let uv0 = new cc.Vec3(pos.x/this.node.width,.0,pos.y/this.node.height);

            let uv = uv0.cross(aspect).mul(1);
            
            let a = uv.x-Math.floor(uv.x);
            let b = uv.y-Math.floor(uv.y);
            let gv = new cc.Vec2(a-.5,b-.5)
            let dropPos = gv.mul(1/5)

            let drop = this.SmoothStep(.0,.1,dropPos.mag())
            cc.log(dropPos)
        })
    ...
    private SmoothStep(edge0,edge1,x){
